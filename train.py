...
def train(gpu, args):
    ...
    checkpoint_file = './tode_checkpoints/checkpoint-epoch26.tar'
    ...
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        for batch, sample in enumerate(train_data_loader):
            ...
            if gpu == 0:
                # sample['pred'] = output['pred'].squeeze(1)
                metric_train = metrics.evaluate_batch(sample, record = False)
                metric_train = torch.tensor(list(metric_train.values())[1:]).cuda().unsqueeze(0)
                writer_train.add(loss_val, metric_train)
                writer_train.add_scalar('Loss/sum', loss_sum.detach().cpu(), GLOBAL_TRAIN_STEP*train_data_loader.batch_size)
                GLOBAL_TRAIN_STEP += 1
                log_cnt += 1
        ...
        for batch, sample in enumerate(val_loader):
            ...
            if gpu == 0:
                # sample['pred'] = output['pred'].squeeze(1)
                metric_val = metrics.evaluate_batch(sample, record = False)
        ...
        for batch, sample in enumerate(test_loader):
            ...
            if gpu == 0:
                # sample['pred'] = output['pred'].squeeze(1)
                metric_test = metrics.evaluate_batch(sample, record = False)
                metric_test = torch.tensor(list(metric_test.values())[1:]).cuda().unsqueeze(0)
