...
import OpenEXR
import numpy as np
from tode_utils.functions import get_surface_normal_from_depth, get_xyz
from tode_utils.constants import DILATION_KERNEL
...
def handle_depth(depth, depth_gt, depth_gt_mask):
    ...
    # depth_gt_mask_uint8 = np.where(depth_gt_mask < 1, 255, 0).astype(np.uint8)
    ...
    depth[mask==1] = depth_uint8[mask == 1] # inpaint with background depth
...
def process_data(
    ...
    **kwargs):
    ...
    h, w = depth.shape
    x_scale = w / original_size[1]
    y_scale = h / original_size[0]
    fx = camera_intrinsics[0, 0] * x_scale
    fy = camera_intrinsics[1, 1] * y_scale
    cx = camera_intrinsics[0, 2] * x_scale
    cy = camera_intrinsics[1, 2] * y_scale
    fx = torch.tensor(fx)
    fy = torch.tensor(fy)
    cx = torch.tensor(cx)
    cy = torch.tensor(cy)
    ...
    data_dict['depth_gt_sn'] = get_surface_normal_from_depth(data_dict['depth_gt'].unsqueeze(0), fx.unsqueeze(0), fy.unsqueeze(0), cx.unsqueeze(0), cy.unsqueeze(0),original_size=original_size).squeeze(0)
    # data_dict['depth_gt_sn'] = get_surface_normal_from_depth(data_dict['depth_gt'].unsqueeze(0), data_dict['fx'].unsqueeze(0), data_dict['fy'].unsqueeze(0), data_dict['cx'].unsqueeze(0), data_dict['cy'].unsqueeze(0),original_size=original_size).squeeze(0)
    # data_dict['xyz_gt'] = get_xyz(data_dict['depth_gt'].unsqueeze(0), torch.tensor(camera_intrinsics[0, 0]).unsqueeze(0).clone(), torch.tensor(camera_intrinsics[1, 1]).unsqueeze(0).clone(), torch.tensor(camera_intrinsics[0, 2]).unsqueeze(0).clone(), torch.tensor(camera_intrinsics[1, 2]).unsqueeze(0).clone(),original_size=original_size).squeeze(0)
    data_dict['xyz_gt'] = get_xyz(data_dict['depth_gt'].unsqueeze(0), fx.unsqueeze(0), fy.unsqueeze(0), cx.unsqueeze(0), cy.unsqueeze(0),original_size=original_size).squeeze(0)
