...
import logging
import numpy as np
from tode_utils.logger import ColoredLogger
from tode_utils.functions import display_results
...
class Metrics(object):
    ...
    def MSE(self, pred, gt, zero_mask, *args, **kwargs):
        ...
        The MSE metric.
        """
        sample_mse = torch.sum(((pred - gt) ** 2) * zero_mask.float(), dim = [1, 2]) / (torch.sum(zero_mask.float(), dim = [1, 2]) + self.epsilon) * self.depth_scale * self.depth_scale
    ...
    def MaskedMSE(self, pred, gt, zero_mask, gt_mask, *args, **kwargs):
        ...
        sample_masked_mse = torch.sum(((pred - gt) ** 2) * mask.float(), dim = [1, 2]) / (torch.sum(mask.float(), dim = [1, 2]) + self.epsilon) * self.depth_scale * self.depth_scale
    ...
    def REL(self, pred, gt, zero_mask, *args, **kwargs):
        ...
        The REL metric.
        """
        sample_rel = torch.sum((torch.abs(pred - gt) / (gt + self.epsilon)) * zero_mask.float(), dim = [1, 2]) / (torch.sum(zero_mask.float(), dim = [1, 2]) + self.epsilon)
    ...
    def MaskedREL(self, pred, gt, zero_mask, gt_mask, *args, **kwargs):
        ...
        """
        mask = gt_mask & zero_mask
        sample_masked_rel = torch.sum((torch.abs(pred - gt) / (gt + self.epsilon)) * mask.float(), dim = [1, 2]) / (torch.sum(mask.float(), dim = [1, 2]) + self.epsilon)
    ...
    def MAE(self, pred, gt, zero_mask, *args, **kwargs):
        ...
        The MAE metric.
        """
        sample_mae = torch.sum(torch.abs(pred - gt) * zero_mask.float(), dim = [1, 2]) / (torch.sum(zero_mask.float(), dim = [1, 2]) + self.epsilon) * self.depth_scale
    ...
    def MaskedMAE(self, pred, gt, zero_mask, gt_mask, *args, **kwargs):
        ...
        """
        mask = gt_mask & zero_mask
        sample_masked_mae = torch.sum(torch.abs(pred - gt) * mask.float(), dim = [1, 2]) / (torch.sum(mask.float(), dim = [1, 2]) + self.epsilon) * self.depth_scale
