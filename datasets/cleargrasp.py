...
from tode_utils.data_preparation import process_data, exr_loader
import cv2
...
class ClearGraspRealWorld(Dataset):
    ...
    def __getitem__(self, id):
        ...
        return process_data(rgb, depth, depth_gt, depth_gt_mask, camera_intrinsics, scene_type = "cluttered", camera_type = 0, split = self.split, image_size = self.image_size, depth_min = self.depth_min, depth_max = self.depth_max, depth_norm = self.depth_norm, use_aug = False, original_size = original_size)
...
class ClearGraspSynthetic(Dataset):
    ...
    def __init__(self, data_dir, split = 'train', **kwargs):
        ...
        #     self.image_paths[i] = self.image_paths[0]
        #     self.mask_paths[i] = self.mask_paths[0]
        #     self.depth_gt_paths[i] = self.depth_gt_paths[0]
        #     self.json_paths[i] = self.json_paths[0]
    ...
    def __getitem__(self, id):
        ...
        depth_gt_mask = cv2.resize(depth_gt_mask, self.image_size, interpolation = cv2.INTER_NEAREST)
        depth_gt_mask = depth_gt_mask.astype(np.bool)
        # a = ip_basic.fill_in_multiscale(depth)
        # a = ip_basic.fill_in_fast(depth)
        # depth = handle_depth2(depth)
        # depth[depth_gt_mask==1] = a[0][depth_gt_mask==1]
        return process_data(rgb, depth, depth_gt, depth_gt_mask, camera_intrinsics, scene_type = "cluttered", camera_type = 0, split = self.split, image_size = self.image_size, depth_min = self.depth_min, depth_max = self.depth_max, depth_norm = self.depth_norm, use_aug = self.use_aug, rgb_aug_prob = self.rgb_aug_prob, use_depth_aug = self.use_depth_aug, original_size = original_size)
