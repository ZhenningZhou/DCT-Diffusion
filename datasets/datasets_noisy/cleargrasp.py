...
class ClearGraspSynthetic(Dataset):
    ...
    def __getitem__(self, id):
        if self.split == 'train':
            ...
            depth_path = self.depth_gt_paths[id]
            depth_path = depth_path.replace(depth_path.split('/')[-2],'depth-imgs-noisy')
            depth_path = depth_path.replace('exr', 'npy')
            ...
            # focal_length = camera_intrinsics[0][0] * 2.0/9.0
