...
from tode_utils.data_preparation import process_data
# from tode_utils.noise_depth_generation import process_data, exr_loader
...
class OmniverseObject(Dataset):
    ...
    def __init__(self, data_dir, split = 'test', **kwargs):
        ...
        # self.split_ratio = kwargs.get('split_ratio', 1.0)
    ...
    def __getitem__(self, id):
        ...
        depth_path = self.h5_paths[id]
        if '20200904' in depth_path.split('/'):
            depth_path = depth_path.replace('20200904','20200904_noisy_gt').replace('h5','npy')
        elif '20200910' in depth_path.split('/'):
            depth_path = depth_path.replace('20200910','20200910_noisy_gt').replace('h5','npy')
        else:
            raise Exception('wrong path')
        depth_gt = np.load(depth_path, allow_pickle=True) #corrupted depth
        ...
        # focal_length = camera_intrinsics[0][0] /3.0
        ...
        #                     , depth_path = depth_path, focal_length = focal_length)
        return process_data(rgb, depth, depth_gt, depth_gt_mask, camera_intrinsics, scene_type = "cluttered", camera_type = 0, split = self.split, image_size = self.image_size, depth_min = self.depth_min, depth_max = self.depth_max, depth_norm = self.depth_norm, use_aug = self.use_aug, rgb_aug_prob = self.rgb_aug_prob, use_depth_aug = self.use_depth_aug)
