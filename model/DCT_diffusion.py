...
class Diffusion_SeparateTODEbase_Model(nn.Module):
    def __init__(
            self,
            args,
        ...
        super(Diffusion_SeparateTODEbase_Model, self).__init__()
        """
        ...
        if depth_head is None:
            ...
            if args.head_specify is not None:
                depth_head_cfg=dict(type=args.head_specify,
                                # in_channels=[64, 128, 256, 512],  # resnet18
                                in_channels=[24, 48, 96, 192],  # resnet18
    ...
    def extract_depth(self, fp, img, depth_map, depth_mask, gt_depth_map, return_loss, img_metas, weight_map=None,
    ...
    def forward(self, sample):
        ...
        depth_mask = sample['depth_gt_mask'] 
        # print(depth_mask)
        # print(depth_map)
        feat = sample['feat']
        output_dict = self.extract_depth(feat, img_inputs, depth_map, depth_mask, gt_depth_map, img_metas=None,
                                       return_loss=True, weight_map=None, instance_masks=None, sparse_depth=None)
        return output_dict
